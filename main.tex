...
Our experimental setup involves classifying a balanced, supervised test set of 300 texts, using two different strategies: \textbf{Zero-Shot} prompting, where the model is given the task description and input text only, and \textbf{Few-Shot} prompting, where the model is provided with the task description and a small set of labeled examples.
...
\begin{itemize}
    ...
    \item Mistral proves capable of following the syntactic constraints in the base configuration for most examples, but tends to default to the \textit{animosity} class in the zero-shot setting.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Prompt Construction:} We use a strict template defining the role of the model, listing the five valid categories, and instructing the model to output \textit{only} the label. The prompt consists of the instructions, the target text and, for the Few-Shot setting, the labelled examples.
        %\begin{itemize}
    ...
    \item \textbf{Output Parsing:} The raw text response is mapped to numerical IDs (0–4). If the model generates invalid text, the system defaults to \textbf{0} (\textit{not-sexist}).
\end{enumerate}
...
It is crucial to note that TinyLlama's metrics are artifacts of the pipeline defaulting invalid outputs to class \textbf{0}. While Prompt Tuning successfully corrected the syntax, it resulted in a semantic collapse where the model constrained its predictions almost exclusively to \textit{not-sexist} and \textit{threats}.
This shift yielded only a marginal performance increase (Accuracy 0.23, F1 0.12), which shows how the model switched from a fallback-driven default to a learned prediction bias, without improving discrimination.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=1.0\linewidth]{conf_matrix.png}
  \caption{Confusion matrices for Mistral in Zero-Shot and Few-Shot Inference.}
