...
This work compares two open-source LLMs, \textit{TinyLlama-1.1B-Chat-v1.0} and \textit{Mistral-7B-Instruct-v0.3}, on multiclass sexism detection using zero-shot and few-shot prompting. Results highlight a critical dependency on model size: TinyLlama consistently failed to follow instructions in both inference settings and, even after performing parameter-efficient Prompt Tuning, the formatting errors were corrected, but semantic reasoning was still absent, leading to a single-class prediction bias. In contrast, Mistral-7B demonstrated effective instruction adherence. Although initially biased towards generic \textit{animosity}, Mistral's performance improved significantly with few-shot examples, raising the F1-score from 0.35 to 0.46.
%\end{quote}
\end{abstract}
...
Our experimental setup involves classifying a balanced, supervised test set of approximately 300 texts \textcolor{red}{[ADD INFORMATION ABOUT DATASET/REFERENCE?]} using two different strategies:
\begin{enumerate}
    \item \textbf{Zero-Shot:} The model is given the task description and input text only.
    \item \textbf{Few-Shot:} The model is provided with the task description and a small set of labeled examples.
\end{enumerate}
...
\begin{itemize}
    \item The smaller model (TinyLlama, 1.1B parameters) lacks the capacity to strictly follow formatting constraints out-of-the-box.
...
\begin{enumerate}
    \item \textbf{Prompt Construction:} We use a strict template defining the role of the model, listing the five valid categories, and instructing the model to output \textit{only} the label.
        \begin{itemize}
            \item Zero-Shot: Instruction + target text.
            \item Few-Shot: Instruction + labeled examples + target text.
        \end{itemize} 
...
To address TinyLlama limitations, we implemented an additional Prompt Tuning pipeline for such model. This technique involves freezing the pre-trained model backbone and optimizing only a small set of continuous vector embeddings (soft prompts) that are prepended to the input (corresponding to approx. 0.006\% of parameters).
The goal of this procedure is to align the model with the specific prompt structure to enforce valid label generation.
...
The performance evaluation of the models is carried out by considering the Macro F1 Score and the Accuracy Score. The following tables summarize the performance metrics obtained during the standard prompting experiments, for Zero-Shot and Few-Shot Inference respectively.\\

\begin{table}[h]
\centering
\caption{Zero-Shot Inference Metrics}
\begin{tabular}{@{}lll@{}}
\toprule
...
\begin{table}[h]
\centering
\caption{Few-Shot Inference Metrics}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Model} & \textbf{F1-Score} & \textbf{Accuracy} \\ \midrule
Mistral        & 0.46              & 0.48              \\
...
It is important to mention that, for both inference settings, the reported metrics for TinyLlama are artifacts of the evaluation pipeline rather than true performance. Because the model consistently failed to output valid labels, the parsing logic defaulted every prediction to class 0 ('not-sexist'). Consequently, the model achieved 20\% accuracy simply by matching the ground truth for that specific class by chance, reflecting the dataset distribution rather than any successful classification.\\

\textbf{Prompt Tuning (TinyLlama)}\\
Prior to prompt tuning, TinyLLaMA consistently failed to follow instructions, producing long free-form text. After fine-tuning, the model successfully learned to respect the output format constraint, generating only valid integer labels. However, this did not translate to classification success. The tuned model exhibited a strong prediction bias, assigning the class “not-sexist” (0) across almost all inputs, resulting in metrics effectively identical to the baseline fallback performance.
...
\textcolor{red}{SHOW EXAMPLE OF WRONG CLASSIFICATION}
...
Ultimately, the prompt tuning experiment was successful in making the model learn the syntax, as all 300 examples in the test dataset were classified with a single, valid label; nevertheless, the model still failed to learn the semantics, leading to unsatisfying performance once again. This dual failure—first in formatting, then in discrimination—strongly suggests that the 1.1B parameter size acts as a fundamental bottleneck, limiting both the inhibitory control required for instruction following

Concerning Mistral, the model presents a strong bias towards the “animosity” class in the Zero-Shot setting, which, however, was significantly mitigated by few-shot prompting, as shown by the two confusion matrices in Figure~\ref{fig:conf_matrix}
